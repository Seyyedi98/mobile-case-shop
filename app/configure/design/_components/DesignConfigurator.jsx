...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }) => {
  ...
  function base64ToBlob(base64, mimeType) {
    ...
    // Do it to convert to blob element. What is blob????
    ...
    <div className="relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
      ...
      >
        <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
          ...
          {/* Picture gray effect outside of mobile page */}
          <div
          ...
          {/* Case color */}
          <div
            style={{ background: `${options.color.tw}` }}
            className={cn(
        ...
        <Rnd
          ...
          // set dimensions and position of the picture when user changes them
      ...
      <div className="h-[37.5rem] w-full  col-span-full lg:col-span-1 flex flex-col bg-white">
        <ScrollArea className="relative flex-1 overflow-auto">
          {/* White shadon at the end of scroll area */}
          <div
            area-hidden="true"
                  ...
                  <div className="mt-3 flex items-center space-x-3">
                    {COLORS.map((color) => (
                      <RadioGroup.Option
                        ...
                        style={{
                          borderColor:
                            color.value === options.color.value
                              ? `${color.tw}`
                              : "",
                        }}
                        className={({ active, checked }) =>
                          cn(
                            ...
                            {
                              //  borderColor: `${color.tw}` : active || checked,
                            }
                          )
                        }
                      >
                        <span
                          style={{ background: `${color.tw}` }}
                          className={cn(
                    ...
                    <DropdownMenuContent>
                      {MODELS.options.map((model) => (
                        <DropdownMenuItem
                          style={{
                            backgroundColor:
                              model.label === options.model.label && "#f4f4f5",
                          }}
                          key={model.label}
                        ...
                        <div className="mt-3 space-y-4">
                          {selectableOptions.map((option) => (
                            <RadioGroup.Option
                              ...
                              style={{
                                borderColor:
                                  option.value === options.material.value ||
                                  option.value === options.finish.value
                                    ? "#e11d48"
                                    : "",
                              }}
                              className="relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm
                              border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between"
                            >
